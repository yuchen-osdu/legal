...
import org.opengroup.osdu.legal.util.AzureServiceBusHelper;
...
public class TestDeleteLegalTagApiAcceptance extends DeleteLegalTagApiAcceptanceTests {
    ...
    public void should_receiveSubscriptionMessage_when_deletingAContractThatDoesExist() throws Exception {
        ...
        while (AzureServiceBusHelper.getMessage(AzureLegalTagUtils.getAzureServiceBusConnectionString(),
            AzureLegalTagUtils.getAzureServiceBusTopicName(), subscriptionName, 1) != null) {}
        ...
        int i = 0;
        while (i < 10 && !passed) {
            IMessage message = AzureServiceBusHelper.getMessage(AzureLegalTagUtils.getAzureServiceBusConnectionString(),
            AzureLegalTagUtils.getAzureServiceBusTopicName(), subscriptionName, 2);
            passed = AzureServiceBusHelper.checkLegalTagNameSent(message, name);
            ++i;
            Thread.sleep(1000);
        }
